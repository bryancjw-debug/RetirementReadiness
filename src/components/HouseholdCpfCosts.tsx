...
export function HouseholdCpfCosts({ rows }: { rows: HouseholdYear[] }) {
  return <details className="year-data-card cpf-help">
    ...
    <div className="table-wrap household-year-table"><table>
      <thead><tr><th>Year</th><th>Person / age</th><th>Top-up paid</th><th>Top-up unfilled</th><th>Premium total</th><th>From MA</th><th>Cash required</th><th>Additional cash expense</th><th>Housing cash</th></tr></thead>
      <tbody>{rows.flatMap(row => row.people.map(person => <tr key={`${row.calendarYear}-${person.id}`}>
        ...
        {[person.cpfRetirementTopUp, person.cpfRetirementTopUpUnfilled, person.insurancePremiumTotal, person.cpfMaMedicalPremium, person.insuranceCashPremium, person.insuranceCashExpense, person.housingCashPayment].map((amount, index) => <td key={index}>{formatCurrency(amount)}</td>)}
      </tr>))}</tbody>
    </table></div>
    <div className="household-year-cards">{rows.flatMap(row => row.people.map(person => <article key={`${row.calendarYear}-${person.id}`}>
      ...
      <dl>{[
        ...
        ["Cash required", person.insuranceCashPremium], ["Additional cash expense", person.insuranceCashExpense], ["Housing cash", person.housingCashPayment]
